...
export default function PhotoGallery({ photos, isLocked, projectPassword }: PhotoGalleryProps) {
  ...
  const downloadPhoto = (photo: Photo) => {
    if (isLocked) {
      console.warn('Downloads locked until payment is complete.')
      return
    }
    // Simulate download
          ...
          <div className="max-w-5xl max-h-full">
            ...
            <div className="text-center mt-4 space-y-2">
              ...
              <Button
                ...
                variant="outline"
                disabled={isLocked}
                className="bg-white/10 text-white hover:bg-white/20 border-white/30 disabled:opacity-60 disabled:cursor-not-allowed"
              >
                <Download className="h-4 w-4 mr-2" />
                {isLocked ? 'Pay to download' : 'Download'}
              </Button>
            </div>
          </div>
        </div>
